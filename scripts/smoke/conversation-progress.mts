...
import { withToolProgress } from "../../src/core/tool-progress.js";
...
assert(baseUrl, "Set SICLAW_TEST_MODEL_URL to the authorized chat-completions base URL");
...
const reports = [];
const api = process.env.SICLAW_TEST_API ?? "openai-completions";
const thinkingLevel = process.env.SICLAW_TEST_THINKING ?? "off";
const configurations = process.env.SICLAW_TEST_MATRIX === "1"
  ? [{ api: "openai-completions", thinkingLevel: "off" }, { api: "openai-responses", thinkingLevel: "high" }]
  : [{ api, thinkingLevel }];
for (const { api, thinkingLevel } of configurations) for (const mode of ["native", "tools-only"] as const) {
  let executions = 0;
  const domainTool = {
  ...
  const wrapped = withToolProgress(domainTool);
  const events: any[] = [];
  const queue: any[] = [];
  let settled = false;
  ...
  const agent = new Agent({ getApiKey: () => key,
    initialState: {
      systemPrompt: "You are a helpful assistant. This is a fictional test inventory. Briefly explain what you will check in the user's language before tool batches. After results, connect the observed finding to the next check. Never invent evidence or expose private reasoning. Include a brief public update in _siclaw_progress for each tool call. " +
        (mode === "tools-only" ? "For this transport test, put progress ONLY in that field: do not send ordinary assistant text while calling tools. After finishing, send one ordinary final answer." : "Prefer ordinary assistant text alongside tool calls. Finish with one self-contained answer."),
      ...
      tools: [wrapped], thinkingLevel: thinkingLevel as any,
    },
  });
  ...
  const brain = new PiAgentBrain({ agent, subscribe: agent.subscribe.bind(agent) } as any, new Map([["lookup_inventory", "query"]]), new Set(["lookup_inventory"]));
  brain.subscribe(e => {
  ...
  for (const e of relayed) {
    ...
    if (e.type === "progress_update" || (e.type === "message_update" && e.assistantMessageEvent?.type === "text_delta" && e.assistantMessageEvent.delta?.trim())) hasProgress = true;
  ...
  const progress = relayed.filter(e => e.type === "progress_update" || (e.type === "message_end" && e.message?.role === "assistant" && e.phase === "commentary"))
    .map(e => e.text ?? e.message.content.filter((p: any) => p.type === "text").map((p: any) => p.text).join("")).filter(Boolean);
  assert.equal(progress.length, 2, "Exactly one progress paragraph per batch");
