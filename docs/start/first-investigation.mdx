...
If you want Siclaw to inspect a Kubernetes cluster, import a kubeconfig in **Clusters** in the Web UI and bind it to your agent.

## Describe the Problem

Open your agent's chat in the Web UI and enter:
...
For complex issues, enable the magnifying-glass toggle next to the Web UI message input, then describe the problem:

```
Intermittent 5xx errors on the API gateway, happening every 30 minutes
```
...
```
Service mesh intermittent 503 errors between order-service and inventory-service
```

Enable Deep Investigation for complex cross-service issues.
