...
## The Command Prints Help or Requires a Prompt

The interactive terminal interface has been removed. Use `siclaw local` for the Web UI, or `siclaw --prompt "..."` for one diagnostic run. `--continue` also requires a new `--prompt`.

If a run reports missing providers, configure **Models** in the local Web UI, or prepare `.siclaw/config/settings.json` for a standalone CLI run. The CLI no longer starts a setup wizard.
...
Open **Clusters** in the Web UI, import the kubeconfig, and bind it to your agent. A CLI invocation paired with the same local Portal picks it up on its next run.
