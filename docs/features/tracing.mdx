...
One web user question retains its Trace ID across Agent ownership transfers.
The source captures the `transfer_to_agent` tool span before eviction; the
receiving AgentBox creates its own `agent.prompt` span with that remote parent.
Multiple hops, including a return to the original Agent, keep the same trace ID
and have distinct spans. An ordinary next question starts a new trace.

This context travels through the trusted handoff control envelope. It is not a
model tool parameter or a public trace override. The prompt ACK and persisted
chat rows use the inherited ID. With export disabled or no attached recorder,
the audit ID is still inherited, but no spans are emitted by that AgentBox.
When only an ID is available, no actual source tool span can be linked.

Upgrade both the external portal and SiClaw. Old events without context remain accepted, but
trace continuity is not guaranteed and existing records are not merged. This
covers the web handoff loop: the external portal `/run` and A2A handlers do not yet drive
an equivalent transfer loop. Export remains best effort; this change does not
instrument every HTTP, WebSocket or database operation in the control plane.
