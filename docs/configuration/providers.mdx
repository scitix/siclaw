...
These override the default provider's settings at runtime (Gateway/Docker mode only):

```bash
SICLAW_API_KEY=sk-...       # Override default provider's API key
SICLAW_BASE_URL=https://... # Override default provider's base URL
```

For TUI mode, configure providers via the first-run wizard or `/setup` command.
