...
This document describes Siclaw's **consumer/runtime** contract: what reaches a
bound agent, and the trust rules that gate it. Authoring and publication are
owned by the upstream knowledge service. It records contracts and rationale,
not call order.

## Producer → consumer shape

The knowledge publisher owns three projections, deterministically regenerated after
...
belongs to the knowledge publishing service and is out of scope here. The flat headless CLI+Portal
