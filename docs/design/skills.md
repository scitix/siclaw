...
### Why not ad-hoc commands?

The agent can already run `kubectl` and other whitelisted commands directly. Skills exist because:

1. **Reliability** — tested scripts avoid the trial-and-error of ad-hoc command generation.
2. **Safety** — scripts go through security review (static + AI) before publication.
3. **Reusability** — investigation patterns are encoded once, used by all users.
4. **Governance** — the approval workflow ensures scripts meet the team's quality bar
   before they enter the shared pool.
