...
### 1.5 ⚠️ Critical: An Unanswered Replica Count Is Not a Replica Count of One

**Invariant**: `agents.replicas` arrives over RPC (`config.getAgent` on the control-plane WS). That call can fail, and a failure carries **no information about the pool's correct size**. Nothing that DESTROYS a box may treat it as a size, because sizing and existence are different questions with different failure directions: guessing high costs a pod, guessing low costs a running pool.

**Why the lookup fails is not one thing.** All of these produce the same rejected promise:
- the WS is not connected — `FrontendWsClient.request` rejects immediately, it does not queue. The drain reaper starts with `setBoxStatusProbe` and does **not** wait for the control plane, so a Runtime restart puts its first tick 10 s in, while the connection may still be coming up or the control plane may itself be rolling.
- the RPC times out (control plane slow, restarting, or partitioned).
- the control plane answers but refuses the agent — its record was removed, renamed, or bound elsewhere.

**Consequences**:
- `lookupReplicas` answers `undefined` for "cannot establish". Only `resolveReplicas` — the serving path — may read that as one box: a request naming the agent is evidence that this Runtime serves it, and one box is the safe shape for work that has to happen. **Destruction requires an answer; service does not.**
- `reconcilePoolSizes` checks it as the first statement of its loop body, ahead of crash healing, staleness marking, roll advance and shrink. Anywhere later and the destruction has already happened.
- **A drain mark records WHY, and an `excess` mark is withdrawn once the pool is not over its count.** This is the half that a check at the marking site cannot cover: marks are acted on ticks after they are made, so a lookup that fails for one tick and recovers leaves a shrink queued against a pool that was never surplus. `boxes.length`, not `accepting.length`, is what the withdrawal compares — `accepting` has already had the marks subtracted, so it can only ever answer "no". `stale` and `unresponsive` marks are judged from what the box itself presents and are re-reached on re-examination, so withdrawing them would only churn the drain budget.
- `reapDrainedBoxes` re-confirms before `stop()`, and DROPS a mark whose agent has become unestablishable. A held mark would keep a box out of its own pool's `accepting` set forever, since every later tick answers unknown too.
- A failed lookup is **not** cached. A cached unknown would make the serving path hold a multi-replica agent at one box for the whole TTL, sending every new session to instance 0. Its two costs are paid separately instead: log noise by rate-limiting the LINE, and the repeated RPC by a memo (`UNOWNED_MEMO_MS`) that `reconcilePoolSizes` alone reads.
- Nothing here restores a pool. `reconcilePoolSizes` heals crashes and advances rolls but never re-fills a shrink; a wrongly shrunk pool comes back only when a request reaches `getOrCreatePooled`. On a quiet agent it stays short.
- `spawner.list()` is scoped to the namespace and the `<prefix>/app=agentbox` label. Pods carry `agent`, `boxType`, `instance`, `ca-fp` and `cert-exp` but **no runtime identity**, so if several Runtimes ever share one AgentBox namespace, each sees the others' pods and would judge them by its own replica count, image and CA. The rules above are what make that safe; there is no label to filter on, and adding one cannot be retroactive.
- Not scoped: `sweepOrphans`' capability-box branch asks this Runtime's `isLive` about a run. Its oracle already fails safe on a thrown store lookup (`catch { return true }`); the remaining gap is a store that answers null for a run it does not own. Its chat-box branch removes terminal pods only.

**Source**: `src/gateway/agentbox/manager.ts` (`lookupReplicas`, `resolveReplicas`, `reconcilePoolSizes`, `withdrawExcessDrains`, `reapDrainedBoxes`), `src/gateway/frontend-ws-client.ts` (`request` rejects when not connected)
