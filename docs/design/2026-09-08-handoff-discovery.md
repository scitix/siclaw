...
`search_handoff_targets` asks SiCore for complete binding matches, with exact cluster
...
identity and accepts only query fields. SiCore verifies Runtime ownership, permitted
...
full previous-routing brief is serialized into tool definitions. SiCore rechecks
...
Deploy SiCore first. Older control planes can still return full index metadata,
