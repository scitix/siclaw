...
| `src/core/model-routing.ts` (buffered runner) + routed prompt paths in `src/agentbox/http-server.ts` / `session.ts` | — | `npm test` (`model-routing.test.ts`, `http-server.test.ts`) | Routed brain events bypass `_eventBuffer` and reach SSE via the extra channel; buffering ends at the FIRST tool execution (fallback is blocked there — events stream live from that point, a failed attempt's pre-tool output stays invisible); setup-failure exhaustion MUST synthesize an assistant `message_end` with `stopReason:"error"` (chat clients build error bubbles only from that shape — zero brain events = silent empty turn); exhausted runs RESOLVE (only `prompt_error` rejects) — callers must not log/score them as success |
