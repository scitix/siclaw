...
1. **准备 / Plan(对话 + 提计划)**:跟负责人聊清楚这个 KB 要收什么、口径、边界、脱敏要求、待解问题,并维护 `authoring/INTENT.md` / `PLAN.md`。**此阶段绝不写 `candidate/` 页面、不大批产出。** 当你已读懂 `raw/`、且和负责人对齐后,**先把打算产出的候选页逐条以 `- [ ] 页名 — 一句话` 复选框写进 `authoring/PLAN.md` 的 `## Next Pages` 小节**(负责人 UI 的计划展示与「批准 · 开始编译」按钮由这个文件驱动——只在聊天里说计划,UI 不会出现批准控件),再调 `propose_plan` 抛出一份**简短、可读、可审核**的编译计划(打算产出哪些候选页、各页一句话、关键口径如脱敏、仍待定的点),**然后停下等批准**——不要擅自开编。
...
def _make_compile_tools(run: CompileRun):
    ...
    @tool(
        "propose_plan",
        "Plan 阶段对齐后调用:把一份简短、可读、可审核的编译计划抛给负责人请求批准,然后停下等批准。"
        "前置:先把候选页清单以 `- [ ] 页名 — 一句话` 复选框写进 authoring/PLAN.md 的 ## Next Pages"
        "(负责人 UI 的计划与批准按钮由它驱动)。在收到批准前不要写 candidate/ 页面。",
        {"plan": str},
    )
    async def propose_plan(args):
        ...
        if "- [ ]" not in section and "- [x]" not in section:
            return {"content": [{"type": "text", "text": (
                "计划文本已收到,但 authoring/PLAN.md 的「## Next Pages」小节还没有 `- [ ] 页名 — 一句话` "
                "复选框清单 —— 负责人 UI 的计划展示与「批准 · 开始编译」按钮由这个文件驱动,没写等于负责人看不到计划。"
                "请先把打算产出的候选页逐条写成复选框(模板里的普通圆点说明行不算),再重新调用 propose_plan。"
            )}]}
        await run.emit({"type": "plan_proposed", "plan": args.get("plan", "")})
        return {"content": [{"type": "text", "text": "计划已抛给负责人(UI 已能看到计划清单与批准按钮),等待批准。在收到批准消息前不要写 candidate/ 页面。"}]}
