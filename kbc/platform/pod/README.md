...
- **Repair**: `turn_done` still fires as usual (the never-stuck invariant holds); when something is unaccounted, one repair instruction is injected (`KBC_L1_REPAIR_ROUNDS`, default 1); once the budget is spent the report is marked `unconverged` and the rest is left to the owner. Fail-open throughout.
