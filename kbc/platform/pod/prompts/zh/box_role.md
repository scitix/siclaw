...
- `authoring/` 存准备阶段资产:AGENTS.md(旧库可能叫 CLAUDE.md,读存在的那个)、manifest.yaml、INTENT.md、PLAN.md、QUESTIONS.md、LEDGER.md;另有 `EXCLUSIONS.json`(你维护的排除声明,见下"覆盖账本")、`BRIEF.json`(系统据开编消息里的定调标签写的本次编译 brief,你读它、不用写它,见下"编译 brief")和 `SELFCHECK.json` / `CONSUMER_META.json`(系统写的自检结果与消费侧摘要,这两个都不要动)。
