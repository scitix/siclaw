...
{{"summary": "≤{summary_max} characters, one or two sentences: what this KB is + when to consult it — never a list of its contents",
 "not_for": ["≤{not_items} topics EXPLICITLY out of scope, ≤{not_chars} characters each — ONLY what the wiki text itself declares as not covered, or the grounded exclusion candidates listed below (if any); NEVER an adjacent domain you merely infer; a false 'not covered' silently routes agents away from real answers, so when in doubt leave this list EMPTY"],
...
{{"summary": "≤{summary_max} 字,一到两句:这个库是什么 + 什么时候来查——绝不罗列内容清单",
 "not_for": ["≤{not_items} 条明确排除的主题,每条 ≤{not_chars} 字——只写 wiki 正文自己声明不覆盖的,或下方给出的有据排除候选(如有);绝不写你推断出来的相邻领域;一条错误的「不覆盖」会把 agent 从真答案边上引开,所以只写有依据的排除,宁缺勿滥,允许为空"],
 "topics": ["3-8 个主题关键词"],
...
# Grounded not_for candidates from the compile-time exclusion ledger. Injected
# by CODE (the blind session's read root stays candidate/-only); reasons ONLY —
# patterns are raw file paths, and echoing a source name in the published
# routing note would advertise the existence of unpublished raw material.
EXCLUSIONS_BLOCK_EN = """

Grounded exclusion candidates (owner-readable reasons from the compile-time exclusion ledger — you may distill not_for items from these; rephrase each as a topic boundary and NEVER quote any file/source name):
{reasons}"""

EXCLUSIONS_BLOCK_ZH = """

有据的排除候选(来自编译期排除台账的负责人可读理由——not_for 可从中提炼;每条改写成主题边界,绝不得出现任何文件名/来源名):
{reasons}"""
...
# Bound the injected grounding: reasons are one-liners; a pathological ledger
# must not blow up the prompt.
_EXCLUSION_REASONS_CAP = 10
_EXCLUSION_REASON_CHARS = 60


def _exclusion_candidates(workdir: str) -> list[str]:
    """Owner-readable `reason` strings from authoring/EXCLUSIONS.json, read by
    CODE — the blind session's read root stays candidate/-only. These ground
    not_for: a negative claim ("this KB does not cover X") must trace to an
    explicit basis, never an inferred adjacent domain. Reasons ONLY: patterns
    are raw file paths/globs and never reach the prompt (a not_for echoing a
    source name would leak the existence of unpublished raw material)."""
    entries, _errors = selfcheck.load_exclusions(workdir)  # malformed file → just no grounding
    out: list[str] = []
    for e in entries:
        r = str(e.get("reason") or "").strip()[:_EXCLUSION_REASON_CHARS]
        if r and r not in out:
            out.append(r)
        if len(out) >= _EXCLUSION_REASONS_CAP:
            break
    return out
...
async def generate_consumer_meta(engine: ReadonlyAgentEngine, *, workdir: str,
    ...
    reasons = _exclusion_candidates(workdir)
    if reasons:
        user += (EXCLUSIONS_BLOCK_EN if en else EXCLUSIONS_BLOCK_ZH).format(
