...
export function registerAdapterRoutes(router: RestRouter, internalSecret: string): void {
  ...
  router.get("/api/internal/siclaw/agent/:agentId/settings", async (req, res, params) => {
    ...
    const [modelRows] = await db.query(
      `SELECT model_id, name, reasoning, vision, context_window, max_tokens, api_type, max_tokens_field, compat_overrides
...
export function buildAdapterRpcHandlers(): Map<string, (params: any, agentId: string) => Promise<any>> {
  ...
  handlers.set("config.getAgent", async (params) => {
    ...
    return {
      ...
      // Agent type (sre/coordinator/custom). Built-in types lock capabilities;
  ...
  /**
   * "This row entered processing" — give it its place in the conversation.
   *
   * Separate from chat.updateMessage because that method replaces the row's columns from
   * its payload: a call carrying only an id would blank the message. Idempotent: a
   * replayed echo must not move a row that already has a place.
   */
  handlers.set("chat.sequenceMessage", async (params) => {
    await getDb().query(
