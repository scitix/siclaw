...
import type { SiclawConfig, TracingConfig, TracingExporterConfig } from "../../core/config.js";
...
/**
 * Build the OTel resource attributes for the provider. Pure (no IO) so the
 * attribute mapping is unit-testable; its only caller is initTracing.
 *
 * `deployment.environment.name` is the OTel semconv key Langfuse reads to bucket
 * traces by environment (it also accepts `deployment.environment` /
 * `langfuse.environment`). Sourced per-runtime from SICLAW_TRACING_ENVIRONMENT
 * (config.ts env override → config.tracing.environment), forwarded to every
 * agentbox by the k8s-spawner allowlist. Omitted when unset (Langfuse then
 * defaults the trace to the `default` environment).
 */
export function buildResourceAttributes(tracing: TracingConfig): Record<string, string> {
  return {
    "service.name": tracing.serviceName ?? DEFAULT_SERVICE_NAME,
    ...(tracing.environment ? { "deployment.environment.name": tracing.environment } : {}),
  };
}
...
export function initTracing(config: SiclawConfig): void {
  ...
  const resource = resourceFromAttributes(buildResourceAttributes(tracing));
  const built = new NodeTracerProvider({ resource, spanProcessors: processors });
  built.register();
  provider = built;
  enabled = true;
  const envSuffix = tracing.environment ? `, environment=${tracing.environment}` : "";
  console.log(`[tracing] enabled with ${processors.length} exporter(s), service.name=${tracing.serviceName ?? DEFAULT_SERVICE_NAME}${envSuffix}`);
}
