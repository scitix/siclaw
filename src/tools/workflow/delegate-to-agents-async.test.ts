...
import { createDelegateToAgentsAsyncTool, registration } from "./delegate-to-agents-async.js";
...
describe("delegate_to_agents_async", () => {
  it("stays hidden until an async runtime executor is injected", () => {
  ...
  it("resolves as a user-approved workflow tool when executor is present", () => {
    ...
    const tools = reg.resolve({
      mode: "web",
      refs: stubRefs({ delegateToAgentsAsyncExecutor: vi.fn() }),
    });

    expect(tools).toHaveLength(1);
    expect(tools[0].name).toBe("delegate_to_agents_async");
  ...
  it("starts a normalized background batch and returns immediately with running details", async () => {
    const executor = vi.fn(async () => ({
      ...
      total_tasks: 2,
      tasks: [
        { index: 1, status: "running" as const, agent_id: "self", scope: "check nodes" },
        { index: 2, status: "running" as const, agent_id: "network-doctor", scope: "check network" },
      ],
    }));
    const tool = createDelegateToAgentsAsyncTool(stubRefs({ delegateToAgentsAsyncExecutor: executor }));
  ...
  it("rejects empty, oversized, and blank-scope task batches", async () => {
    const tool = createDelegateToAgentsAsyncTool(stubRefs({ delegateToAgentsAsyncExecutor: vi.fn() }));
