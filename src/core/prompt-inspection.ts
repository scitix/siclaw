...
function designChecks(input: {
  ...
  if (harness.agentType === "knowledge_qa") {
    ...
    const preservesExploration = retrievalContract.includes("complete page catalog") &&
      retrievalContract.includes("navigation metadata, not answer evidence") &&
      retrievalContract.includes("never searches page bodies");
    add(
      ...
      !forcesSearch && preservesExploration
        ? "The complete catalog remains primary; label resolution is navigation metadata and leaf pages provide evidence."
        : "Knowledge QA is missing the complete-catalog, labels-only, or leaf-evidence contract.",
    );
  }
