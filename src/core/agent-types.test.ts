...
describe("agent-types", () => {
  it("has the six designed types; built-ins lock capabilities and own their runtime contracts", () => {
    expect(Object.keys(AGENT_TYPES).sort()).toEqual(["coordinator", "custom", "knowledge_qa", "product_support", "siforge-agent", "sre"]);
    ...
    expect(AGENT_TYPES["siforge-agent"].defaultPrompt).toBe(SIFORGE_AGENT_DEFAULT_PROMPT);
    expect(AGENT_TYPES["siforge-agent"].defaultNoSkills).toBe(false);
    expect(AGENT_TYPES.custom.capabilities).toBeNull();
    expect(AGENT_TYPES.custom.defaultPrompt).toBeNull();
  });

  it("siforge-agent locks byte-identical capabilities to sre", () => {
    ...
    expect(AGENT_TYPES["siforge-agent"].capabilities).toEqual(AGENT_TYPES.sre.capabilities);
  });
  ...
  it("normalizeAgentType defaults unknown/absent to custom", () => {
    ...
    expect(normalizeAgentType("siforge-agent")).toBe("siforge-agent");
  ...
  it("requireAgentType accepts every registered type — it is a hand-written second enumeration", () => {
    ...
    expect(requireAgentType("siforge-agent")).toBe("siforge-agent");
  });
