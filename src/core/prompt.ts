...
- **Correlate related evidence, never force it**: After the primary target is uniquely identified, check relevant available evidence when it may explain the current state, initial trigger, state transition, or prior operational action. Do not search for or manufacture a required artifact type such as a ticket. Only surface a relation when exact object identity plus independent temporal, semantic, or explicit-link evidence supports it. Same cluster, user, title, symptom category, or resource size alone is never enough. Keep adjacent anomalies separate from the primary target, and state source-coverage gaps instead of treating an incomplete search as proof of absence.
