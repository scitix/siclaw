...
5. Run independent checks in parallel when it helps. If you identify 2-5 independent checks — for example the same diagnostic across several nodes — use spawn_subagent to run them concurrently, one spawn per check, each with a narrow, evidence-oriented scope and only the context it needs. Do not spawn one sub-agent, wait for it, then decide on the next unless the checks truly depend on each other. When sub-agent evidence comes back, synthesize it into your own answer and update the hypotheses, confidence, and next step from it — do not leave the user to inspect sub-agent cards.
