...
function FileEntry({ name, content }: { name: string; content: string }) {
    const [expanded, setExpanded] = useState(name === 'SKILL.md');
        ...
        <div className="border-t border-gray-100">
            ...
            {expanded && (
                <div className="px-4 pb-3">
                    <pre className="text-xs font-mono leading-relaxed text-gray-600 whitespace-pre-wrap bg-gray-50 rounded p-3 max-h-[60vh] overflow-y-auto">{content}</pre>
                </div>
            )}
        </div>
    );
...
export function SkillPreviewPanel({ message, onClose }: SkillPreviewPanelProps) {
    ...
    // Build flat file list: SKILL.md first, then scripts
    const files: Array<{ name: string; content: string }> = [];
    if (skill.specs) {
        files.push({ name: 'SKILL.md', content: skill.specs });
    }
    if (skill.scripts) {
    ...
    return (
        <div className="w-[480px] border-l border-gray-200 bg-white flex flex-col shrink-0 h-full">
            ...
            <div className="flex-1 overflow-y-auto min-h-0">
                {files.map((f) => (
                    <FileEntry key={f.name} name={f.name} content={f.content} />
                ))}
            </div>
        </div>
    );
