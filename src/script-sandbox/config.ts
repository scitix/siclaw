...
export function loadScriptSandboxConfig(env: NodeJS.ProcessEnv = process.env): ScriptSandboxConfig {
  ...
  let mcpPolicy: ScriptSandboxConfig["mcpPolicy"] = {};
  if (env.SICLAW_SCRIPT_SANDBOX_MCP_POLICY_FILE) {
    mcpPolicy = JSON.parse(readFileSync(env.SICLAW_SCRIPT_SANDBOX_MCP_POLICY_FILE, "utf8"));
  ...
  const hostKeyPins = env.SICLAW_SCRIPT_SANDBOX_HOST_KEYS_FILE ? JSON.parse(readFileSync(env.SICLAW_SCRIPT_SANDBOX_HOST_KEYS_FILE, "utf8")) : {};
  if (!hostKeyPins || Array.isArray(hostKeyPins) || typeof hostKeyPins !== "object" || Object.values(hostKeyPins).some(v => typeof v !== "string" || !/^SHA256:[A-Za-z0-9+/]{43}$/.test(v))) throw new ScriptSandboxError("Invalid sandbox host key pins");
  return {
    hostKeyPins,
