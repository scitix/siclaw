...
export class ScriptSandboxPool implements ScriptSandboxProvider {
  ...
  prewarm(): void {
    ...
    this.timer ??= setInterval(() => {
      for (const mode of this.idle.keys()) {
        ...
        for (const entry of expired) void entry.channel.close().catch(() => {});
  ...
  private fill(mode: boolean): void {
    if (this.stopped || mode !== this.preferredMode || this.filling.has(mode) || !this.config.warmPoolSize) return;
    const task = (async () => {
      const entries = this.idle.get(mode)!;
      while (!this.stopped && mode === this.preferredMode && entries.length < this.config.warmPoolSize) {
        ...
        if (this.stopped || mode !== this.preferredMode) { await channel.close(); return; }
  ...
  async start(runId: string, isolated: boolean, timeoutSeconds: number, signal: AbortSignal): Promise<ScriptChannel> {
    ...
    this.preferredMode = isolated;
    // An idle incompatible profile must not occupy the only available slot
    // while a real request waits. Running instances have already left the pool.
    const retired = this.idle.get(!isolated)!.splice(0);
    if (retired.length) await Promise.all(retired.map(entry => entry.channel.close()));
    if (this.stopped || signal.aborted) throw new Error("Script execution cancelled");
  ...
  async shutdown(): Promise<void> {
    ...
    await Promise.allSettled([...this.idle.values()].flat().map(e => e.channel.close()));
    this.idle.clear();
